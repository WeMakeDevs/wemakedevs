	...
	if (!isLoaded) {
		return (
			<div className="flex justify-center items-center py-20">
				<div className="text-lg text-slate-400">Loading mission timeline...</div>
			</div>
		);
	}
				...
				{groupedSchedule.map((group, index) => (
					<button
						...
						className={cn(
							...
							"focus:outline-none",
							selectedDate === index
								? "bg-gradient-to-r from-sky-500 to-amber-500 text-white hover:from-sky-400 hover:to-amber-400"
						...
						return (
							...
							>
								...
								{totalItems > 1 && (
									<>
										{index === 0 && (
											<div className="absolute h-[100%] w-1 top-1/2 left-[14px] bg-sky-500/30 z-0" />
										)}
										{index === totalItems - 1 && index > 0 && (
											<div className="absolute h-[50%] w-1 top-0 left-[14px] bg-sky-500/30 z-0" />
										)}
										{index > 0 && index < totalItems - 1 && (
											<div className="absolute h-[145%] w-1 top-0 left-[14px] bg-sky-500/30 z-0" />
										)}
									</>
								)}

								{/* Timeline dot */}
								<div className="rounded-full p-1.5 bg-sky-500/50 shadow-lg shadow-sky-500/30 relative z-10">
									<div className="rounded-full bg-gradient-to-br from-sky-400 to-amber-500 shadow-md h-5 w-5" />
								</div>
								...
								{isTextItem ? (
									...
									<div className="px-4 py-2 ring-2 ring-sky-500/20 w-full hover:ring-sky-500/40 transition-all bg-slate-800/50 rounded-lg">
										<h3 className="text-lg font-body font-semibold md:text-xl text-sky-400">
											{item.title}
										</h3>
										...
										<div className="flex gap-2 md:gap-5 items-center font-medium flex-wrap mt-2">
											...
											{("links" in item &&
												...
												(item.link && (
													<Link
														href={item.link.url}
														className="hover:underline no-underline text-sky-400 flex items-center gap-2 uppercase"
														target="_blank"
														rel="noopener noreferrer"
													>
														<Link2 size={16} />
