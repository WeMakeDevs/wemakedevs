...
const NeonDeLorean = () => (
	...
	>
		...
		<path
			d="M528,95 L556,84 L552,90 L578,82"
			stroke="#00ffff"
			strokeWidth="1.8"
			fill="none"
			opacity="0.85"
		...
		/>
		<path
			d="M528,112 L558,106 L554,112 L580,107"
			stroke="#b400e4"
			strokeWidth="1.8"
			fill="none"
			opacity="0.75"
		...
		/>
		<path
			d="M528,130 L552,124 L548,130 L572,125"
			stroke="#FF9D00"
			strokeWidth="1.2"
			fill="none"
			opacity="0.65"
		...
		/>
		<path
			d="M528,148 L548,144 L544,149 L564,146"
			stroke="#FF9D00"
			strokeWidth="0.9"
			fill="none"
			opacity="0.4"
			...
			{lineageEdges.map((e, i) => {
				...
				return (
					<line
						...
						style={{
							animation: `drawLineage 1.6s ease forwards ${0.6 + i * 0.12}s`,
						}}
					/>
				);
			})}
			...
			{lineageNodes.map((n, i) => (
				<g key={`n-${i}`}>
					...
					<circle
						...
						filter="url(#nodeGlow)"
						style={{
							animation: `pulseNode 2.4s ease-in-out infinite ${n.delay}s`,
...
const HeroSection = ({
	...
	return (
		<div
			ref={containerRef}
			className="relative min-h-[62svh] md:min-h-[68svh] overflow-hidden flex flex-col"
		>
			{/* Parallax star layer */}
			<motion.div
				style={{ y: starsY }}
